...
Figure 1: Layered architecture of the AI Execution Layer within AI-native systems, illustrating the separation between cognition, the Contact Layer, and capability execution.
...
- `P` represents the **Contact Layer** (routing and decision-making),
...
**Contact Layer (P):** The Contact Layer determines *how* capabilities are selected and composed. This layer is responsible for:
...
**Execution Runtime (Invocation):**
- API invocation
- Tool execution
- Error handling
- Result normalization

This decomposition maintains clear separation of concerns. The Contact Layer makes *decisions* (which provider, which environment), while the execution runtime performs *actions* (invoke API, return result).
...
*Table 1: Cross-Runtime Performance (DeepSeek API)[^1]*

| Implementation | Throughput (RPS) | Avg Latency (ms) | P99 Latency (ms) |
|----------------|------------------|------------------|------------------|
| ai-lib-rust    | 99.1 ± 0.4       | 50.0 ± 0.2       | 201 ± 3          |
| ai-lib-python  | 98.0 ± 0.5       | 51.0 ± 0.2       | 210 ± 4          |
| ai-lib-ts      | 99.7 ± 0.2       | 49.6 ± 0.1       | 199 ± 2          |

*Table 2: Provider Portability Validation (Groq API)[^2]*

| Implementation | Throughput (RPS) | P50 Latency (ms) | P99 Latency (ms) |
|----------------|------------------|------------------|------------------|
| ai-lib-rust    | 17.9 ± 1.5       | 195 ± 2          | 1518 ± 274       |
| ai-lib-python  | 20.4 ± 2.6       | 186 ± 1          | 1038 ± 348       |
| ai-lib-ts      | 18.0 ± 0.9       | 195 ± 3          | 1611 ± 59        |

[^1]: Test Configuration: DeepSeek API (`deepseek-chat`) / 10s duration × 3 runs / 5 concurrent connections / generate_text capability / 100 max tokens. Tested 2026-02-25.

[^2]: Test Configuration: Groq API (`llama-3.1-8b-instant`) / 10s duration × 3 runs / 5 concurrent connections / generate_text capability. Tested 2026-03-13.

**Key Findings:**

1. **Cross-Runtime Consistency:** All three implementations achieve statistically equivalent throughput (within 2% variance) when targeting the same provider, validating protocol-level consistency.

2. **Provider Portability:** The same codebase successfully connects to multiple LLM providers (DeepSeek, Groq) without modification, demonstrating the capability abstraction model in practice.

3. **Production Viability:** Throughput exceeds 99 RPS on DeepSeek and 17+ RPS on Groq, with tail latencies suitable for interactive applications.
...
The `retry_count` and `translation_latency_ms` fields provide visibility into execution behavior, enabling the Contact Layer to make data-driven routing decisions.
...
**Stateless Constraint Clarification.** The runtime itself remains stateless: it acts purely as a protocol translator and forwarder, pushing standardized events to the Contact Layer, which maintains all subscription state and routing policy.
...
2. **Economic Modeling.** How do pricing models influence routing strategies at the Contact Layer?
...
- *No built-in policy logic.* Routing decisions, fallback chains, and cost optimization must be implemented by the Contact Layer, not the runtime itself.
...
- *Orchestration maturity.* Organizations without Contact Layer expertise may not fully realize the separation benefits.
