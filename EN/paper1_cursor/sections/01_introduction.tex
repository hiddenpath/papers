...
The execution layer is then positioned within the broader AI-Native architecture developed in later sections. \Cref{fig:layered-architecture-overview} summarizes the layered boundary, while later figures refine the implementation stack and execution flows.
...
\node[box, fill=blue!8] (agent) { \textbf{Agent Intelligence Layer}\\(Reasoning, Planning, Cognition) };
\node[box, fill=green!8, below=0.95cm of agent] (orch) { \textbf{Policy Orchestration Layer}\\(Routing, Provider Selection, Workflow Control) };
\node[box, fill=orange!10, below=0.95cm of orch] (exec) { \textbf{AI Execution Layer (Execution Runtime)}\\Protocol-to-API Translation \quad $\bullet$ \quad Result Normalization \quad $\bullet$ \quad Error Propagation };
\node[boundary, below=0.85cm of exec] (boundary) {--- AI Protocol Boundary ---};
\node[box, fill=purple!8, below=0.85cm of boundary] (prov) { \textbf{Capability Providers / Tools / Devices}\\(LLMs, APIs, Databases, External Services, IoT) };
